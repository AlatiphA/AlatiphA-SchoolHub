...
function loadImageViaCanvas(url) {
  // Loads a URL through a plain <img> element and normalizes it to a PNG
  // data URL via canvas. This works regardless of the original upload's
  // format (JPEG/WEBP/HEIC-converted/etc.) and is what makes the image
  // safe to hand to jsPDF's addImage — no format guessing needed.
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        if (!canvas.width || !canvas.height) { reject(new Error('Image has no dimensions.')); return; }
...
async function reportImageToDataUrl(source) {
  ...
  // Primary path: <img> + canvas. Firebase Storage download URLs send
  // Access-Control-Allow-Origin: *, so this isn't canvas-tainted, and it
  // is far more reliable on mobile browsers (Samsung Browser included)
  // than raw fetch()/blob reading, which can be blocked or return an
  // opaque response depending on tracking-prevention settings.
  try {
    return await loadImageViaCanvas(value);
  } catch (canvasError) {
    console.warn('Report image canvas load failed, trying direct fetch:', value, canvasError);
  }

  // Fallback: fetch the bytes directly and read them as a data URL.
  try {
    ...
    console.warn('Report image could not be loaded at all:', value, err);
    return '';
  }
}
...
async function resolveReportAsset(primarySource, fallbackFolder, fallbackPrefixes, cache) {
  // Reuse a result already resolved earlier in this same PDF generation —
  // the school logo and both signatures are identical for every student in
  // a batch, so without this they'd otherwise be re-fetched from scratch
  // once per student.
  const cacheKey = primarySource || `${fallbackFolder}|${(fallbackPrefixes || []).join(',')}`;
  ...
  if (!result) {
    ...
    result = url ? await reportImageToDataUrl(url) : '';
  }
  if (cache) cache.set(cacheKey, result);
  return result;
}

async function prepareReportAssets(result, settings, classInfo, cache) {
  const staffList = DB.get(KEYS.staff, []);
...
async function generateSinglePDF(result, positions, numOnRoll, classInfo, studentRemarks, settingsOverride) {
  ...
  let assets;
  try {
    assets = await prepareReportAssets(result, settings, classInfo, assetCache);
  } catch (assetError) {
...
async function generateBatchPDF(results, positions, numOnRoll, classInfo, remarksAll, settingsOverride) {
  ...
  for (let i = 0; i < usable.length; i++) {
    ...
    let assets;
    try {
      assets = await prepareReportAssets(r, settings, classInfo, assetCache);
    } catch (assetError) {
      console.warn('Report image preparation failed:', assetError);
      assets = { logo: '', photo: '', classTeacherSignature: '', headTeacherSignature: '' };
    }
