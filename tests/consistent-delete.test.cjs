...
test('class deletion remains protected while subject deletion requires an explicit score-cascade confirmation',async()=>{
 for(const bulk of [false,true]){
  const f=fixture();f.ctx.bulkSelectionsV40.classes.add('c1');
  if(bulk)await f.ctx.bulkDeleteV40('classes');else await f.ctx.deleteRecordsV40('classes',['c1']);
  assert.equal(f.deleted.length,0);assert.equal(f.writes.length,0);assert.match(f.alerts[0],/Safe Delete blocked/);
 }
 for(const bulk of [false,true]){
  const f=fixture();f.ctx.bulkSelectionsV40.subjects.add('math');
  if(bulk)await f.ctx.bulkDeleteV40('subjects');else await f.ctx.deleteRecordsV40('subjects',['math']);
  assert.equal(f.cascadeCalls.length,1);assert.match(f.confirmations[0],/permanently remove those linked scores/i);
  assert.equal(f.data.subjects.some(s=>s.id==='math'),false);
  assert.equal(Object.hasOwn(f.data.grades.term,'p1'),false);
  assert.equal(Object.hasOwn(f.data.grades.term,'p2'),false);
 }
});
